...
namespace O2S_InsuranceExpertise.GUI.FormCommon.TabCaiDat
{
    public partial class ucDanhSachBenhVien : UserControl
    {
        ...
        private void LoadDanhSachBenhVien()
        {
            try
            {
                string sqldsbv = "select row_number () over (order by benhvienname) as stt, benhvienid, benhvienkcbbd, benhviencode, benhvienname, benhvienaddress from ie_benhvien;";
                    ...
                    if (data != null)
                    {
                        ...
                        for (int i = 0; i < dmCSYT_Import.Count; i++)
                        {
                            if (dmCSYT_Import[i]["BENHVIENKCBBD"].ToString() != "")
                            {
                                ...
                                {
                                    string sql_updateUser = "UPDATE ie_benhvien SET benhviencode='" + dmCSYT_Import[i]["BENHVIENCODE"].ToString() + "', benhvienname='" + dmCSYT_Import[i]["BENHVIENNAME"].ToString() + "', benhvienaddress='" + dmCSYT_Import[i]["BENHVIENADDRESS"] + "' WHERE benhvienkcbbd='" + dmCSYT_Import[i]["BENHVIENKCBBD"].ToString() + "';";
                                    try
                                    {
                                ...
                                {
                                    string sql_insertDVKT = "INSERT INTO ie_benhvien(benhvienkcbbd, benhviencode, benhvienname, benhvienaddress) VALUES ('" + dmCSYT_Import[i]["BENHVIENKCBBD"].ToString() + "', '" + dmCSYT_Import[i]["BENHVIENCODE"].ToString() + "', '" + dmCSYT_Import[i]["BENHVIENNAME"].ToString().Replace("'", "''") + "', '" + dmCSYT_Import[i]["BENHVIENADDRESS"].ToString().Replace("'", "''") + "');";
                                    try
                                    {
