...
namespace O2S_InsuranceExpertise.GUI.MenuTrangChu
{
    ...
    {
        private O2S_InsuranceExpertise.DAL.ConnectDatabase condb = new O2S_InsuranceExpertise.DAL.ConnectDatabase();
        private string currentUserCode;
        private List<O2S_InsuranceExpertise.DTO.classPermission> lstPer { get; set; }
        private List<O2S_InsuranceExpertise.DTO.classUserDepartment> lstUserDepartment { get; set; }
        private List<O2S_InsuranceExpertise.DTO.classPermission> lstPerBaoCao { get; set; }
        private List<O2S_InsuranceExpertise.DTO.classUserMedicineStore> lstUserMedicineStore { get; set; }
        private List<O2S_InsuranceExpertise.DTO.classUserMedicinePhongLuu> lstUserMedicinePhongLuu { get; set; }
        ...
        private void LoadDanhSachNguoiDung()
        {
            try
            {
                string sql = "select usercode, username, userpassword, case usergnhom when '0' then 'Admin' when '1' then 'Quản trị hệ thống' when 2 then 'Nhân viên' end as usergnhom from ie_tbluser where usergnhom in (1,2) order by usercode";
                ...
                {
                    O2S_InsuranceExpertise.Utilities.ThongBao.frmThongBao frmthongbao = new O2S_InsuranceExpertise.Utilities.ThongBao.frmThongBao(O2S_InsuranceExpertise.Base.ThongBaoLable.KHONG_CO_DU_LIEU);
                    frmthongbao.Show();
                }
            }
            catch (Exception ex)
        ...
        private void LoadDanhSachChucNang()
        {
            try
            {
                ...
                lstPer = O2S_InsuranceExpertise.Base.listChucNang.getDanhSachChucNang().Where(o => o.permissiontype != 10).ToList();
        ...
        private void LoadDanhSachBaoCao()
        {
            try
            {
                ...
                lstPerBaoCao = O2S_InsuranceExpertise.Base.listChucNang.getDanhSachChucNang().Where(o => o.permissiontype == 10).ToList();
        ...
        private void LoadDanhSachPhongLuu()
        {
            try
            {
                ...
                for (int i = 0; i < dataPhongLuu.Count; i++)
                {
                    ...
                    userMedicinePhongLuu.MedicinePhongLuuId = Common.TypeConvert.TypeConvertParse.ToInt32(dataPhongLuu[i]["medicinephongluuid"].ToString());
                    ...
                    userMedicinePhongLuu.MedicineStoreId = Common.TypeConvert.TypeConvertParse.ToInt32(dataPhongLuu[i]["medicinestoreid"].ToString());
        ...
        private void CreateNewUser(string en_txtUserID, string en_txtUsername, string en_txtUserPassword)
        {
            try
            {
                ...
                {
                    sqlinsert_user = "INSERT INTO ie_tbluser(usercode, username, userpassword, userstatus, usergnhom, usernote) VALUES ('" + en_txtUserID + "','" + en_txtUsername + "','" + en_txtUserPassword + "','0','1','');";
                }
                else
                {
                    sqlinsert_user = "INSERT INTO ie_tbluser(usercode, username, userpassword, userstatus, usergnhom, usernote) VALUES ('" + en_txtUserID + "','" + en_txtUsername + "','" + en_txtUserPassword + "','0','2','Nhân viên');";
                }
        ...
        private void CreateNewUserDepartment(string en_txtUserID)
        {
            try
            {
                ...
                for (int i = 0; i < lstUserDepartment.Count; i++)
                {
                    ...
                    {
                        sqlinsert_userdepartment = "INSERT INTO ie_tbluser_departmentgroup(departmentgroupid, departmentid, departmenttype, usercode, userdepgidnote) VALUES ('" + lstUserDepartment[i].departmentgroupid + "','" + lstUserDepartment[i].departmentid + "','" + lstUserDepartment[i].departmenttype + "','" + en_txtUserID + "','');";
        ...
        private void CreateNewUserMedicineStore(string en_txtUserID)
        {
            try
            {
                ...
                for (int i = 0; i < lstUserMedicineStore.Count; i++)
                {
                    ...
                    {
                        sqlinsert_usermedicinestore = "INSERT INTO ie_tbluser_medicinestore(medicinestoreid, medicinestoretype, usercode, userdepgidnote) VALUES ('" + lstUserMedicineStore[i].MedicineStoreId + "','" + lstUserMedicineStore[i].MedicineStoreType + "','" + en_txtUserID + "','');";
        ...
        private void CreateNewUserMedicinePhongLuu(string en_txtUserID)
        {
            try
            {
                ...
                for (int i = 0; i < lstUserMedicinePhongLuu.Count; i++)
                {
                    ...
                    {
                        sqlinsert_usermedicinephongluu = "INSERT INTO ie_tbluser_medicinephongluu(medicinephongluuid, medicinestoreid, usercode, userdepgidnote) VALUES ('" + lstUserMedicinePhongLuu[i].MedicinePhongLuuId + "','" + lstUserMedicinePhongLuu[i].MedicineStoreId + "','" + en_txtUserID + "','');";
        ...
        private void UpdateUser(string en_txtUserID, string en_txtUsername, string en_txtUserPassword)
        {
            try
            {
                ...
                if (cbbUserNhom.Text == "Quản trị hệ thống")
                {
                    sqlupdate_user = "UPDATE ie_tbluser SET usercode='" + en_txtUserID + "', username='" + en_txtUsername + "', userpassword='" + en_txtUserPassword + "', userstatus='0', usergnhom='1', usernote='' WHERE usercode='" + en_txtUserID + "';";
                }
                else
                {
                    sqlupdate_user = "UPDATE ie_tbluser SET usercode='" + en_txtUserID + "', username='" + en_txtUsername + "', userpassword='" + en_txtUserPassword + "', userstatus='0', usergnhom='2', usernote='Nhân viên' WHERE usercode='" + en_txtUserID + "';";
                }
            ...
            try
            {
                ...
                for (int i = 0; i < lstPer.Count; i++)
                {
                    ...
                    if (dvkt.Count > 0) //Nếu có quyền đó rồi thì Update
                    {
                        ...
                        {
                            sqlupdate_per = "DELETE FROM ie_tbluser_permission WHERE usercode='" + en_txtUserID + "' and permissioncode='" + en_permissioncode + "' ;";
        ...
        private void UpdateUserDepartment(string en_txtUserID)
        {
            try
            {
                ...
                for (int i = 0; i < lstUserDepartment.Count; i++)
                {
                    ...
                    string sqlkiemtratontai = "SELECT * FROM ie_tbluser_departmentgroup WHERE usercode='" + en_txtUserID + "' and departmentid='" + lstUserDepartment[i].departmentid + "' ;";
                    DataView dvkt = new DataView(condb.GetDataTable_HSBA(sqlkiemtratontai));
                    if (dvkt.Count > 0) //Nếu có quyền đó rồi thì Update
                    {
                        if (lstUserDepartment[i].departmentcheck == false) //xoa
                        {
                            sqlupdate_userdepartment = "DELETE FROM ie_tbluser_departmentgroup WHERE usercode='" + en_txtUserID + "' and departmentid='" + lstUserDepartment[i].departmentid + "' ;";
                    ...
                    else //nếu không có quyền đó thì Insert
                    {
                        if (lstUserDepartment[i].departmentcheck == true)
                        {
                            sqlupdate_userdepartment = "INSERT INTO ie_tbluser_departmentgroup(departmentgroupid, departmentid, departmenttype, usercode, userdepgidnote) VALUES ('" + lstUserDepartment[i].departmentgroupid + "','" + lstUserDepartment[i].departmentid + "','" + lstUserDepartment[i].departmenttype + "','" + en_txtUserID + "','');";
                            condb.ExecuteNonQuery_HSBA(sqlupdate_userdepartment);
                        }
                    }
                }
        ...
        private void UpdateUserMedicineStore(string en_txtUserID)
        {
            try
            {
                ...
                for (int i = 0; i < lstUserMedicineStore.Count; i++)
                {
                    ...
                    string sqlkiemtratontai = "SELECT * FROM ie_tbluser_medicinestore WHERE usercode='" + en_txtUserID + "' and medicinestoreid='" + lstUserMedicineStore[i].MedicineStoreId + "' ;";
                    ...
                    if (dvkt_medi.Count > 0) //Nếu có quyền đó rồi thì Update
                    {
                        ...
                        {
                            sqlupdate_usermedicinestore = "DELETE FROM ie_tbluser_medicinestore WHERE usercode='" + en_txtUserID + "' and medicinestoreid='" + lstUserMedicineStore[i].MedicineStoreId + "' ;";
                    ...
                    else //nếu không có quyền đó thì Insert
                    {
                        if (lstUserMedicineStore[i].MedicineStoreCheck == true)
                        {
                            sqlupdate_usermedicinestore = "INSERT INTO ie_tbluser_medicinestore(medicinestoreid, medicinestoretype, usercode, userdepgidnote) VALUES ('" + lstUserMedicineStore[i].MedicineStoreId + "','" + lstUserMedicineStore[i].MedicineStoreType + "','" + en_txtUserID + "','');";
                            condb.ExecuteNonQuery_HSBA(sqlupdate_usermedicinestore);
                        }
                    }
                }
        ...
        private void UpdateUserMedicinePhongLuu(string en_txtUserID)
        {
            try
            {
                ...
                for (int i = 0; i < lstUserMedicinePhongLuu.Count; i++)
                {
                    ...
                    string sqlkiemtratontai = "SELECT * FROM ie_tbluser_medicinephongluu WHERE usercode='" + en_txtUserID + "' and medicinephongluuid='" + lstUserMedicinePhongLuu[i].MedicinePhongLuuId + "' ;";
                    DataView dvkt_medi = new DataView(condb.GetDataTable_HSBA(sqlkiemtratontai));
                    if (dvkt_medi.Count > 0) //Nếu có quyền đó rồi thì Update
                    {
                        if (lstUserMedicinePhongLuu[i].MedicinePhongLuuCheck == false) //xoa
                        {
                            sqlupdate_usermedicinephongluu = "DELETE FROM ie_tbluser_medicinephongluu WHERE usercode='" + en_txtUserID + "' and medicinephongluuid='" + lstUserMedicinePhongLuu[i].MedicinePhongLuuId + "' ;";
                    ...
                    else //nếu không có quyền đó thì Insert
                    {
                        if (lstUserMedicinePhongLuu[i].MedicinePhongLuuCheck == true)
                        {
                            sqlupdate_usermedicinephongluu = "INSERT INTO ie_tbluser_medicinephongluu(medicinephongluuid, medicinestoreid, usercode, userdepgidnote) VALUES ('" + lstUserMedicinePhongLuu[i].MedicinePhongLuuId + "','" + lstUserMedicinePhongLuu[i].MedicineStoreId + "','" + en_txtUserID + "','');";
                            condb.ExecuteNonQuery_HSBA(sqlupdate_usermedicinephongluu);
                        }
                    }
                }
        ...
        void itemXoaNguoiDung_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                try
                {
                    ...
                    O2S_InsuranceExpertise.Utilities.ThongBao.frmThongBao frmthongbao = new O2S_InsuranceExpertise.Utilities.ThongBao.frmThongBao("Đã xóa bỏ tài khoản: " + currentUserCode);
                    frmthongbao.Show();
