...
from (select hosobenhanid,vienphiid,departmentgroupid,departmentid,bhytid from medicalrecord where doituongbenhnhanid=1 /*and medicalrecordstatus<>99*/ and departmentgroupid=3 and thoigianvaovien between '2017-01-01 00:00:00' and '2017-01-04 00:00:00') mrd
	inner join (select bhytid,bhytcode,macskcbbd,bhytdate,bhytfromdate,bhytutildate,bhyt_loaiid,noisinhsong,du5nam6thangluongcoban,dtcbh_luyke6thang,lastaccessdate from bhyt where bhytcode<>'' and bhytdate between '2017-01-01 00:00:00' and '2017-01-04 00:00:00' and bhytid not in (select IE_bhyt.bhytid
from dblink('myconn_ie','SELECT bhytid FROM IE_bhyt_check where bhytcheckstatus=1 and hosobenhandate between ''2017-01-01 00:00:00'' and ''2017-01-04 00:00:00''') AS IE_bhyt(bhytid integer)) ) bh on bh.bhytid=mrd.bhytid
	inner join (select hosobenhanid,patientid,patientname,hosobenhandate,hosobenhandate_ravien,birthday,birthday_year,gioitinhcode,gioitinhname,lastaccessdate from hosobenhan where /*hosobenhanstatus=0 and */hosobenhandate between '2017-01-01 00:00:00' and '2017-01-04 00:00:00') hsba on hsba.hosobenhanid=mrd.hosobenhanid
